...
class BenchmarkAnalyzer:
    ...
    def plot_time_vs_text_size(self, save: bool = True):
        ...
        for i, dataset in enumerate(datasets):
            for j, hit in enumerate(hit_values):
                ...
                # Filter data
                data = self.stats[
                    (self.stats["Dataset"] == dataset) & (self.stats["Hit"] == hit)
                ]
    ...
    def plot_time_vs_pattern_size(self, save: bool = True):
        ...
        for i, dataset in enumerate(datasets):
            ...
            for algo in data["Algo"].unique():
                ...
                pattern_stats = (
                    algo_data.groupby("PatternSize")
                    .agg({"Time_mean": "mean", "Time_std": "mean"})
                    .reset_index()
                )
    ...
    def plot_memory_usage(self, save: bool = True):
        ...
        for i, dataset in enumerate(datasets):
            ...
            for algo in data["Algo"].unique():
                ...
                # Group by text size
                size_stats = (
                    algo_data.groupby("TextSize")
                    .agg({"Memory_mean": "mean"})
                    .reset_index()
                )
            ...
            if (
                has_data
                and data["Memory_mean"].max() / max(data["Memory_mean"].min(), 1) > 10
            ):
                ax.set_yscale("log")
        ...
        if save:
            ...
            if not has_data:
                print("     Memory measurements are very small (< 100 bytes)")
                print("     This is expected for string matching algorithms.")
                print("     Memory usage represents Python object allocation overhead.")
    ...
    def plot_hit_vs_miss_comparison(self, save: bool = True):
        ...
        fig, axes = plt.subplots(
            len(algos), len(datasets), figsize=(6 * len(datasets), 5 * len(algos))
        )
        ...
        for i, algo in enumerate(algos):
            for j, dataset in enumerate(datasets):
                ...
                # Filter data
                data = self.stats[
                    (self.stats["Algo"] == algo) & (self.stats["Dataset"] == dataset)
                ]
    ...
    def plot_algorithm_comparison_heatmap(
        self, metric: str = "Time", save: bool = True
    ):
        """
        ...
        for i, dataset in enumerate(datasets):
            ...
            ax.set_title(
                f"{dataset} - {metric} Comparison", fontsize=14, fontweight="bold"
            )
        ...
        for i, dataset in enumerate(datasets):
            ...
            for algo in algos:
                ...
                try:
                    ...
                    if best_fit == "O(n)":
                        ax.plot(
                            x, y_linear, "--", label=f"{algo} O(n) fit", linewidth=2
                        )
                    elif best_fit == "O(n log n)":
                        ax.plot(
                            x, y_log, "--", label=f"{algo} O(n log n) fit", linewidth=2
                        )
                    else:  # O(n*sqrt(n))
                        ax.plot(
                            x, y_quad, "--", label=f"{algo} O(n·√n) fit", linewidth=2
                        )
            ...
            ax.set_ylabel("Time (seconds)", fontsize=12, fontweight="bold")
            ax.set_title(
                f"Scaling Analysis - {dataset}", fontsize=14, fontweight="bold"
            )
    ...
    def generate_summary_statistics(self, save: bool = True):
        ...
        algo_comparison = (
            self.stats.groupby("Algo")
            .agg({"Time_mean": "mean", "Memory_mean": "mean"})
            .round(6)
        )

        # Convert memory to KB for readability
        algo_comparison["Memory_KB"] = (algo_comparison["Memory_mean"] / 1024).round(2)
        algo_comparison = algo_comparison.rename(
            columns={"Time_mean": "Avg_Time(s)", "Memory_KB": "Avg_Memory(KB)"}
        )
        ...
        dataset_comparison = (
            self.stats.groupby("Dataset").agg({"Time_mean": "mean"}).round(6)
        )
        ...
        hit_comparison = (
            self.stats.groupby(["Algo", "Hit"]).agg({"Time_mean": "mean"}).round(6)
        )
...
def main():
    ...
    parser.add_argument("csv_file", type=str, help="Path to benchmark results CSV file")
